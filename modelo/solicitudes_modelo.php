...
switch($opcion){
    ...
    case 4://enviar pedido
        ...
        $consulta="SELECT id_pendientes,cantidad, unidad, detalle,archivo,ruta FROM items_pendientes WHERE items_pendientes.id_usuarios='$id_usuario'";
        $resultado = $conexion->prepare($consulta);
        $resultado->execute();
        ...
        foreach($items as $d){
            ...
            $archivo=$d['archivo'];
            $ruta=$d['ruta'];
            $consulta="INSERT INTO items(id_pedido,cantidad,unidad,detalle,archivo,ruta) VALUES ('$ultimoPedido','$cantidad','$unidad','$detalle','$archivo','$ruta')";
            $resultado = $conexion->prepare($consulta);
            $resultado->execute();
        }

        $consulta = "DELETE FROM items_pendientes WHERE id_usuarios='$id_usuario'";		
        $resultado = $conexion->prepare($consulta);
        $resultado->execute();
